...
## 当前能力（T1+T2+T3+T4+T5+T6+T7+T8+T9+T10+T12）
...
**T12**：**后台全量**（独立进程，只监听 tailnet）—— 12 个页面（概览 / 比赛管理 /
房间与数据源 / 切片复核 / 灰信号评审 / 报告 / 发布与回滚 / 会员与订单 / 通知与告警 / 配置 /
审计日志 / 成本与额度，逐条对应设计 §14.1）+ 16 个写操作。**非 tailnet 请求一律 403**
（连登录页都不给看），唯一角色是管理员（PBKDF2 口令哈希存仓库外 `.env`，哈希同时充当
会话签名密钥 → 改口令即所有旧会话失效）。**配置 1 分钟内生效**落在同一个版本号上：
保存配置、登记/删除直播间都递增 `config_version` —— 本进程立刻生效、别的进程最多滞后
60 秒（TTL 缓存）、采集监督进程按版本号重起受影响的子进程并按登记表增/停房间。
**所有写操作留痕**（含登录失败），页面渲染层一个写语句都没有。**凭据不进库**由三道自动
防线拦住（扫描器 + pre-commit + 测试用例）。

不含通知投递（T11，`danmu-intel events` 里的待投递事件已可查）与归档
（见设计 §19 实施分层；T9 的到期提醒与凭据重发走通讯渠道，属 T11）。
...
```bash
# ① 设管理员口令（只把 PBKDF2 哈希写进仓库外 .env；明文既不落盘也不入库）
danmu-intel admin-passwd

# ② 起后台（独立进程；只肯绑 tailnet 地址或 127.0.0.1，0.0.0.0 直接被拒绝启动）
danmu-intel admin --host "$(tailscale ip -4)" --port 8090
# 浏览器打开 http://<tailnet-ip>:8090/admin → 输入口令 → 12 个页面
# 经 https 反代时加 --secure-cookie；只想落本地产物（不推 git、不调 Vercel）加 --no-deploy
```

| 页面（设计 §14.1） | 能做什么 |
|---|---|
| 概览 | 进程健康 / 比赛状态 / 待办 / 配置版本 / 线上发布 |
| 比赛管理 | 加比赛、删比赛（无数据才删得掉）、状态机操作 |
| 房间与数据源 | 登记 / 改 / 删直播间（**1 分钟内对采集生效**） |
| 切片复核 | 人工修正小局边界（**必填理由**，写 `slice.manual` 审计） |
| 灰信号评审 | `escalate` / `discard`（**必填理由**） |
| 报告 | 触发某场某形态的生成与发布、预览全文（管理员完整视图，不受付费墙） |
| 发布与回滚 | 7 项检查结果、手动发布、一键回滚 |
| 会员与订单 | 搜索、人工补开通（必填理由）、撤权、到期批量降级 |
| 通知与告警 | 待投递事件与告警（**投递属 T11**，本页只读） |
| 配置 | 关键词表与门槛（灰信号 N/M/K、终局阈值）、档位价格、收款配置（可写）；时效预算与成本闸门只读展示 |
| 审计日志 | 谁、何时、改了什么（只增不改） |
| 成本与额度 | 解读成本（本场/今日、硬闸、连续失败）+ 链上额度与限速 + 监听游标（扫到哪了） |

除上面表里的写操作外，登录、登录失败、登出也都进 `audit_log`；共 16 个写动作 +
三条鉴权记录。页面渲染层**一个写语句都没有**（测试递归遍历渲染过程里的 SQL 来守这条线）。

隔离与鉴权（AC-7 / NFR-S-3）：

- 独立进程、**只肯绑 tailnet / 回环地址**（`0.0.0.0` 这种「绑所有网卡」的写法会被拒）；
  每个请求再按对端 IP 判一次，非 tailnet → **403**（连登录页都不给看）；
  拿不到合法对端 IP 也拒绝（默认拒绝，不默认放行）。
- 除登录页外一律要签名 cookie（12 小时、HttpOnly + SameSite=Lax）；
  签名不对 / 过期 / 伪造 → 403。口令哈希同时是会话签名密钥。
- 登录按 IP 限流（1 分钟 10 次）。

**配置 1 分钟生效（NFR-T-4 / 设计 §14.3）—— 三件事各管一段**：

| 段 | 机制 | 滞后上限 |
|---|---|---|
| 后台自己（同一进程） | 保存时立刻失效本进程缓存 | 0 秒 |
| 其他进程读配置 | 进程内 60 秒 TTL 缓存 | 60 秒 |
| 采集子进程 | 监督进程每轮读版本号，变了就重起受影响房间的子进程 | 轮询周期（5 秒） |

**版本号是同一条链路的唯一触发信号**：改配置、登记/删除直播间都递增 `config_version`。
`danmu-intel supervise --match-id 1 --from-registry` 让房间集合跟着登记表走 —— 新登记的房间
当场纳入监督、被删掉的房间停子进程并写明原因（`state='removed'`，不算故障、不计入重启上限）。

凭据不进库的三道防线（NFR-S-4 / AC-12）：

```bash
python3 tools/check_no_secrets.py                # ① 扫描器：私钥块 / 0x+64 hex / xprv / 常见 key 前缀
bash deploy/hooks/install.sh                     # ② 装进 git hooks：提交前自动跑，命中即拒绝提交
python3 -m pytest tests/unit/test_check_no_secrets.py   # ③ 测试：塞假私钥必须被抓到
python3 -m pytest tests/e2e/test_admin_server.py        # 跑完一整轮后台，库里搜不到口令明文与哈希
```

框架选型、12 与 11 的出入、tailnet 两层判定、`X-Forwarded-For` 的可信边界与
「版本号为什么连数据源登记一起管」见 [ADR-0019](docs/adr/0019-admin-implementation.md)。
...
后台的测试缝有三条：**纯函数**（`tests/unit/test_admin_auth.py`：口令哈希 / 签名 cookie /
过期与伪造 / tailnet 与回环判定 / 拒绝绑 `0.0.0.0`；`tests/unit/test_admin_pages.py`：
12 条清单写死 + 空库每页都能开 + 渲染零写语句 + 库里带 `<script>` 的字段必被转义）、
**存储与动作**（`tests/unit/test_admin_stores.py` 的登记/评审/删除与版本号，
`tests/unit/test_admin_actions.py` 的 16 个动作成功与失败两条路）、
**HTTP 边界**（`tests/e2e/test_admin_server.py`：真 aiohttp 服务只跑在 127.0.0.1 上，
非 tailnet 403、无 cookie 403、登录/限流/登出/篡改 cookie、12 页逐一 200、
写操作全留痕、配置保存即换版本、跑完一整轮库里搜不到口令明文与哈希）。
...
- **会员身份对外零泄露**（NFR-P-1/P-2）：用户名只用于联系与运营者核对，不进任何公开页面、
  ...
  「某人是不是会员」。
- **后台只在 tailnet 内可达**（NFR-S-3 / AC-7）：进程只肯绑 tailnet/回环地址
