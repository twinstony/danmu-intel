...
快照。规则直出渲染（`rule_render.py`）、解读层（`interpreter.py`）与页面渲染
（`html.py`）都只读它。

`fact_layer_hash` 是**解读层输入的指纹**（设计 §10.3）：把事实层投影成规范 JSON
再取 SHA256。投影里包含比赛信息、算法版本、各小局的切片边界与规则统计、
落盘文件清单，以及**覆盖到的原始记录行的摘要**——原始记录被改动，指纹必然变。
"""
...
def scope_facts(facts: MatchFacts, scope: ReportScope) -> MatchFacts:
    ...
    windows = tuple((game.window.start_ms, game.window.end_ms) for game in covered)
    lines = tuple(
        line
        for line in facts.all_lines
        if any(start <= line.event.ts < end for start, end in windows)
    )
    return replace(facts, games=covered, all_lines=lines, excluded_games=excluded)
